...
→ `.claude/rules/nextjs-cache-testing.md`로 승격
...
**판단**: `vi.mock` + `vi.mocked(Class).mockImplementation(() => ...)` 방식은 Vitest ESM 환경에서 arrow function이 constructor로 사용되는 문제가 있음. 의존성 주입(optional parameter)으로 구조 변경해 해결 — `translateAndSummarize(text, isKorean, client = getDefaultClient())`. 테스트는 mock 클라이언트를 직접 전달하는 방식으로 단순화.
**다시 마주칠 가능성**: 높음 — Anthropic SDK, OpenAI 등 외부 API 클라이언트를 class로 사용하는 서비스마다 동일 패턴 필요.
→ `.claude/rules/api-client-lazy-singleton.md`로 승격
...
→ `.claude/rules/external-api-allsettled.md`로 승격
